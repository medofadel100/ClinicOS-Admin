...
export default function CreateDiscountClient() {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <h1 className="text-3xl font-bold mb-8">{t("createDiscountCode")}</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-slate-700">{t("codeLabel")}</label>
            <input
              type="text"
              name="code"
              placeholder={t("codePlaceholderDiscount")}
              required
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-slate-700">{t("appliesTo")}</label>
            <select
              name="applies_to"
              className="border border-slate-300 rounded-md px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500"
            >
              <option value="both">{t("bothNewRenewal")}</option>
              <option value="new_subscription">{t("newSubscriptionsOnly")}</option>
              <option value="renewal">{t("renewalsOnly")}</option>
            </select>
          </div>
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-sm font-semibold text-slate-700">{t("descriptionInternal")}</label>
          <input
            type="text"
            name="description"
            placeholder={t("campaignNotes")}
            className="border border-slate-300 rounded-md px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-slate-700">{t("typeDiscount")}</label>
            <select
              name="discount_type"
              className="border border-slate-300 rounded-md px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500"
            >
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-slate-700">{t("valueLabel")}</label>
            <input
              type="number"
              name="discount_value"
              step="0.01"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-slate-700">{t("validUntil")}</label>
            <input
              type="datetime-local"
              name="valid_until"
              className="border border-slate-300 rounded-md px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500"
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-slate-700">{t("maxUses")}</label>
            <input
              type="number"
              name="max_uses"
              min="1"
        ...
        <div className="flex justify-end pt-4 border-t border-slate-100">
          ...
          <button
            ...
            disabled={isPending}
          >
            {isPending ? t("creating") : t("createDiscountCode")}
          </button>
        </div>
      </form>
    </div>
